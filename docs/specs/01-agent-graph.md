...
**Fast-path determinístico de estoque** (`_is_stock_intent`, `orchestrator.py`): PRECEDE o sticky — intenção clara de compra/disponibilidade vai ao vendedor mesmo com a conversa "dona" de outro skill (ex.: cliente abre com sintoma → farmaceutico vira owner; depois "tem expec?" deve ir ao vendedor, não ficar preso no farmaceutico via sticky). Gated por `sales.stock_check` ON + vendedor ativo. O regex positivo (`_STOCK_INTENT_RE`) cobre "quanto custa", "vocês têm", "vende", e **"tem &lt;produto&gt;"** (ampliado jun/2026 — pegava "tem dipirona?"/"tem expec?" que o bare-`tem` antes não casava). A guarda NEGATIVA (`_STOCK_NEGATIVE_RE`) defere ao classificador: **sintoma/clínico** ("tem algum **remédio** pra tosse" → triagem), genérico/bula/dose, e "tem X" **não-produto** (desconto/entrega/frete/convênio/"tem que"/"tem como"). ⚠️ Ao mexer no bare-`tem \w+`, manter a guarda negativa em sync — senão pedido de sintoma vaza pro vendedor e mata a triagem clínica.
