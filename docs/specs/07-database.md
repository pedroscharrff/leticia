...
| `medicamentos_anvisa` + `bula_secoes` | Base regulatória compartilhada (ANVISA). Cache só-leitura: populado pelo cold path dos agentes E pela alimentação manual do superadmin (`POST /admin/medicamentos/bulario/consultar` e `/bulario/bulk`) |
...
| `medicamentos_referencia_consultas` | Log/telemetria de cada consulta do agente à base de referência (termo, medicamento casado, seções ativas devolvidas, tenant/skill). Sem FK (telemetria não bloqueia turno). Alimenta o painel "Consultas". Migration 068 |
...
- **Não usar `jsonb_set(col, '{a,b}', ...)` para criar caminho aninhado quando `col` pode estar `{}`.** `jsonb_set` só cria o elemento FINAL do path; se o pai (`a`) não existir, vira NO-OP silencioso. Colunas JSONB com `NOT NULL DEFAULT '{}'` (ex.: `capability_catalog.config_schema`) nascem `{}`, então `COALESCE(col, '{...}')` também não salva — o COALESCE nunca dispara. Para setar config_schema aninhado, **monte o objeto inteiro com `jsonb_build_object`** ou faça `col || '{...}'`. Mordeu nas migs 051 e 057 (config_schema não aparecia no modal de Recursos); corrigido na 058.
