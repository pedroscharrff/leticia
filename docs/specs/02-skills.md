...
│   └── clinical.py     # blocos do farmaceutico (stock_check quando há catálogo)
├── tools/
...
3. **Marker invisível**: `[[HANDOFF:X]]`, `[[ESCALATE]]`, `[[END]]` SEMPRE removidos do texto antes do cliente ver (regex em `_parse_handoff` / `_parse_escalate` / `_parse_end`). `[[END]]` sinaliza fim de atendimento (cliente se despediu sem pedido pendente) → propaga `end_conversation=True` no state; o worker chama `end_session` (closed_at, sem pausar, limpa histórico). Não propaga quando há handoff/escalate/balcão (essas têm prioridade e já finalizam).
   - **Markup nativo de tool-call vazado (DeepSeek/weak)**: modelos OpenAI-compatible fracos às vezes serializam a tool-call no template NATIVO deles (tokens com barra full-width `｜` U+FF5C — ex. `<｜｜DSML｜｜invoke name="transferir_para_especialidade">…`) DENTRO do `content` em vez de devolver `tool_calls` estruturado. Resultado: `response.tool_calls` vem vazio → o runtime trata como texto final → (a) o markup vaza no WhatsApp e (b) o sinal de fluxo se perde (handoff abandonado). `_strip_leaked_tool_markup` (em `_base.py`) roda na MESMA rede de segurança dos `_parse_*`: RECUPERA o sinal (handoff/escalate/end + destino validado contra a allowlist) e REMOVE o markup. Aplicado em `run_skill` E no loop próprio do `vendedor`. Prioridade: sinal de tool real > markup vazado > marcador `[[...]]`. Cf. [[project_deepseek_provider_integration]].
...
Plano: basic.
Tools: `consultar_bula`, `consultar_bula_secao`. **Sempre que há catálogo** (capability `sales.stock_check` ON — modo Sheets/CSV **OU** ERP) também recebe `buscar_produto` para conferir o catálogo ANTES de afirmar que a loja tem o medicamento — bloco `stock_check_block()` (`prompts/clinical.py`) é anexado ao `_SYSTEM`. **Gate = "catálogo existe" (`sales.stock_check`), NÃO "estoque autoritativo" (`inventory.track_stock`)**: em Sheets a bula confirma que o remédio existe no mundo, mas só o catálogo diz se a LOJA o carrega — gatear em `track_stock` (OFF no Sheets) deixava o farmaceutico afirmando "temos X" pela bula e o vendedor desmentia (regressão real, jun/2026). Em pré-atendimento (`sales.stock_check` OFF, sem catálogo) o comportamento histórico é preservado. **Guard-rail "não achou na bula":** quando `sales.pharmacist_validation` está ON, `make_consultar_bula_tool` recebe `not_found_message` (do config da capability, editável por tenant — mig 056); aí o "nenhum registro" da bula retorna uma instrução determinística mandando o agente NÃO inventar dosagem/apresentação e perguntar ao cliente com aquela frase. Default em `_DEFAULT_NOT_FOUND_MESSAGE`. (O safety_guard da SPEC 10 NÃO cobre isso — é passthrough em pré-atendimento; por isso o guard vive na própria tool.)
Quando: sintoma, dúvida farmacêutica, posologia, interações.
Característica: **handoff obrigatório para vendedor** quando cliente sinaliza finalização (`pode finalizar`, `pode mandar`...). Skill não pode "confirmar pedido" — não tem tool para isso. Bônus do `buscar_produto`: popula `cart._search_results_this_turn`, então `availability_guard` (SPEC 10) passa a cobrir alucinação de produto em recomendação clínica também. **Force-recall de estoque (weak-LLM):** havendo catálogo passa `verify_stock_affirmation=has_catalog` (`sales.stock_check`) ao `run_skill`; se a LLM fraca afirmar "temos" sem ter chamado `buscar_produto` no turno, o runtime FORÇA a busca e regenera a resposta (SPEC 10 §força-busca de estoque). No-op p/ Claude/GPT forte e em pré-atendimento.
...
Tools (modo pré-atendimento): `salvar_dados_cliente`, `consultar_pedido`, `registrar_itens_interesse`, `anotar_pedido_balcao`. Condicional: `consultar_cep` (mesma tool do modo normal) quando o tenant marca `cep` em `sales_config.required_fields` — ver Característica "CEP no pré-atendimento" abaixo.
Quando: cliente quer comprar, preço, montar carrinho.
Característica: **bifurca em modo normal vs pré-atendimento** baseado em capability `sales.stock_check`. Tem force-call no pré-atendimento (se LLM "fechou" sem chamar `anotar_pedido_balcao`, força a chamada). **Desde a modularização (2026-06):** vendedor monta prompt via `PromptBuilder` (+`prompts/commerce.py`), roda o loop via `runtime.run_tool_loop`, e o force-call + draft-fallback vivem no `post_loop_hook` passado ao runtime — NÃO foram removidos, só mudaram de lugar. Controle de fluxo é por tools (handoff/escalate/end), com o parser de marcadores como fallback. Handoff no pré-atendimento: a tool só é bindada (destino `farmaceutico`) quando `sales.pharmacist_validation` ON E farmacêutico ativo. **Force-recall de estoque (weak-LLM, modo normal):** passa `StockRecall(suppress_tools=("adicionar_ao_carrinho","finalizar_pedido"))` ao `run_tool_loop` quando `_v_scaffold and not use_preattendimento` — força `buscar_produto` se a LLM fraca afirmar disponibilidade sem ter consultado (SPEC 10 §força-busca de estoque). **Gate de quantidade (weak):** o `domain_tool_gate` (`_order_gate`) também VETA `adicionar_ao_carrinho` de item novo quando o cliente não declarou a quantidade (`_stated_quantity(current_message)` False E `qty<=1` E item não está no carrinho) — instrui a perguntar "Quantas unidades?". Evita a LLM fraca pular a etapa e assumir 1. `qty>1`/ajuste de item passam livres. Reforçado no playbook (step 2) + `_SALES_DISCIPLINE`. **Diretiva de carrinho aberto (weak, modo normal):** o bloco "Carrinho atual do cliente" é descritivo; havendo `cart["items"]` E `_v_scaffold`, anexa-se uma DIRETIVA imperativa VOLÁTIL ("itens JÁ escolhidos; não re-pergunte 'qual o medicamento?', não reinicie; continue da etapa atual") — espelha o que o pré-atendimento já fazia com o rascunho. Sintoma real (jun/2026): cliente "já falamos disso, tem até carrinho aberto" enquanto o bot pedia o nome do remédio com 2 itens no carrinho. Strong path byte-idêntico. Cf. SPEC 08 §tier (andaime nº7).
...
Característica (**fallback determinístico de rascunho**, 2026-06-10): o LLM frequentemente ignora `registrar_itens_interesse` e lista itens só em texto — o cart fica `items=[]` até `anotar_pedido_balcao`, e se o cliente some antes, a recuperação falha. **Fix em 3 camadas:** (1) **Regra 6** no `_SYSTEM_PRE_ATENDIMENTO` promove a instrução a regra absoluta; (2) **extração determinística** — no fim do turno, se `registrar_itens_interesse` e `anotar_pedido_balcao` não foram chamados mas a resposta enumera itens (heurística `_detect_item_listing`), uma chamada Haiku com schema forçado extrai `[{name,qty}]` do diálogo e grava em `cart["items"]` diretamente (sem `just_finalized`, sem `orders`) — o `save_context` persiste normalmente; (3) **métrica `preattend_draft_fallback_total`** (Counter Prometheus, label `tenant_id`) + log `vendedor.draft.extracted_by_fallback`. **Invariantes:** a extração NÃO seta `just_finalized`, NÃO cria order, NÃO altera a resposta ao cliente — é rascunho puro equivalente a `registrar_itens_interesse`. Custo: ~1 chamada Haiku só nos turnos em que o LLM falhou, só em pré-atendimento. **Heurística ampliada (2026-07-01):** `_detect_item_listing` exigia ≥2 linhas tipo bullet pra disparar — item único narrado em prosa corrida (ex.: "Beleza, vou anotar a Dipirona pra você") escapava e o carrinho ficava vazio. Agora dispara com **1 match** de linha-item OU com frase de confirmação (`_ITEM_ACK_PHRASES`: "anotei"/"registrado"/"adicionei"/etc.) combinada a pelo menos um sinal de quantidade (dígito ou numeral por extenso) — evita falso positivo em small talk puro ("Certo, bom dia!").

Característica (**CEP no pré-atendimento**, 2026-07-01): por padrão o pré-atendimento NUNCA coleta endereço (resolvido no balcão humano — SPEC 08 §andaime 4). Mas quando o tenant marca `cep` em `sales_config.required_fields`, a tool `consultar_cep` (mesma do modo normal, `agents/tools/customer.py`) é bindada e `cep_lookup_block()` (`prompts/commerce.py`) é injetado no prompt — sem isso a allowlist do andaime nº4 permitia PERGUNTAR o CEP mas não havia tool nenhuma pra buscar/validar (contradição: LLM pedia CEP e nunca confirmava endereço). **Force-call de CEP (weak-LLM, `_vendedor_post_hook`):** se o cliente informou um CEP (regex `\d{5}-?\d{3}` na última mensagem) e a LLM fraca não chamou `consultar_cep` no turno, força a chamada — mesmo padrão do force-call de fechamento (HumanMessage de instrução interna → reinvoca com tools → executa → reinvoca sem tools). Fail-open: se a LLM não chamar mesmo forçada, mantém a resposta e loga erro (não quebra o turno). Gated por `_v_scaffold` — caminho forte não sofre force-call (só ganha a tool/prompt quando `required_fields` pede CEP, o que é funcionalidade nova, não andaime).
...
- **Não bindar `consultar_cep` no pré-atendimento incondicionalmente.** Só quando `_preattend_collects_cep` (`"cep" in sales_config.required_fields`) — do contrário reintroduz pergunta de endereço em tenants que configuraram pré-atendimento justamente pra NÃO coletar isso (resolvido no balcão).
